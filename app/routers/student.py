...
def create_students(user: schemas.StudentCreate, db: Session = Depends(get_db), current_user: dict = Depends(oauth2.get_current_user)):
    school = db.query(models.School).filter(
        models.School.manager_id == current_user.id).first()
...
@router.put('/{id}')
def update_student(id: int, updated_student: schemas.UserCreate, db: Session = Depends(get_db), current_user: dict = Depends(oauth2.get_current_user)):
    school = db.query(models.School).filter(
        models.School.manager_id == current_user.id).first()
    student = db.query(models.User).filter(
        models.User.role_id == 4, models.User.id == id).first()
